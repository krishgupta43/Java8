...
public class Java8CommonProgrammingQuestion {
	public static void main(String[] args) {
        ...
        List<Student> students = st.stream().filter(student -> student.getRank() > 50 && student.getRank() < 100)
                .collect(Collectors.toList());
       // System.out.println(students);

        //2. Find the Students who stays in Karnataka and sort them by their names

        List<Student> studentsByCity = st.stream().filter(student -> student.getCity().equals("Karnataka"))
        ...
        List<String> deptNames = st.stream()
                .map(Student::getDept)
                .distinct()
                .collect(Collectors.toList());

        Set<String> deptNamesInSet = st
        ...
        List<String> contacts = st.stream()
        ...
        Map<String, List<Student>> studentMap = st.stream()
                .collect(Collectors.groupingBy(Student::getDept));
        System.out.println(studentMap);


        //6. Find the department who is having maximum number of students
        Map.Entry<String, Long> results = st.stream()
        ...
        Map<String, Double> avgStudents = st.stream()
                .collect(Collectors
        ...
        Map<String, Optional<Student>> stdMap = st.stream()
                .collect(Collectors.groupingBy(Student::getDept,
